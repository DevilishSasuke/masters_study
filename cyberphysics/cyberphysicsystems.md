...
* время обработки $T_1 \rightarrow min$, т.е. частота кадров $FPS \rightarrow max$ (т.к. $FPS = \frac{1}{T_1}$) 

при выполнении следующих ограничений:

* $FPS(W,H,n,k) \ge FPS_{min}$ ($FPS_{min}$ задается исходя из задачи)

* $n < n_{max}$

* $mAP(n)\ge mAP_{n_0}-\Delta$, где $n_0$ - начальное количество слоев, а $\Delta$ - допустимое снижение точности работы модели в процентах по сравнению с исходной (задается исходя из характеристик задачи)
...
2. Уменьшить количество слоев до $n < n_0$ следующим образом:
   
   * Установить диапазон поиска $n \in [n_{min}; n_{max}]$ 
     (изначально $n_{min} = 5, n_{max}=n_0$)
   
   * Вычислить $n_{mid} = \frac{n_{min}+n_{max}}{2}$
   
   * Измерить метрики: $mAP(n_{mid}), FPS(n_{mid})$
   
   * При
     
     * $mAP(n_{mid})\ge mAP_{min}$
     
     * $FPS(n_{mid}) \ge FPS_{min}$
     
     уменьшаем глубину модели $n_{max} = n_{mid}$ 
     и запоминаем значение $n_{correct} = n_{mid}$
   
   * Иначе $n = n_{correct}$ (находим последнее удовлетворяющее значение)

### Изменение входных данных

1. Изменить размер входного изображения $W_n × H_n$ по числам, кратным 32 (примеры: уменьшение $640 \rightarrow 416 \rightarrow 320$, увеличение $416 \rightarrow 512 \rightarrow 640$)
...
3. Отметить зависимость: 
   ...
   $T_1∝f_2(Wn​,Hn​)$, $FPS = \frac{1}{T_1}$ (время обработки обратно пропорционально частоте кадорв)
   
   $f_1 - $ функция, описывающая зависимость точности от разрешения входного изображения,
   
   $f_2 - $ функция, описывающая зависимость времени от разрешения входного изображения.​
...
2. Проанализировать зависимости параметров
   ...
   * уменьшение $n \rightarrow$ рост $FPS$, падение точность $mAP$
...
Это обеспечивает достижение показателей, сопоставимых и сходной моделью [1], при значительно меньшей сложности модели, что важно для систем компьютерного зрения, работающих на мобильных и малопроизводительных устройствах.
